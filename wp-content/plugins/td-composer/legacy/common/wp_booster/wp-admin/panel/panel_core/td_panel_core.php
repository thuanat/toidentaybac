...
class td_panel_core {
     ...
     */
    static function load_ajax_box() { //return the view counts
        ...
        foreach (td_global::$all_theme_panels_list[$td_current_panel_spot_id]['panels'] as $panel_id => $panel_array) {
            ...
            if (isset($panel_array['file']) and strpos($panel_array['file'], $td_ajax_calling_file) !== false) {
                ...
                if ($panel_array['type'] == 'in_theme') {
                    ...
                    $file_path = TDC_PATH_LEGACY . '/includes/panel/views/ajax_boxes/' . $td_ajax_calling_file_id  . '/' . $td_ajax_box_id . '.php';
                    if ( file_exists( $file_path ) ) {
                        load_template( $file_path, true);
                    } else {
                        require_once( TDC_PATH_LEGACY_COMMON . '/wp_booster/wp-admin/panel/views/ajax_boxes/' . $td_ajax_calling_file_id . '/' . $td_ajax_box_id . '.php');
                    }
                    $buffy = ob_get_clean();
